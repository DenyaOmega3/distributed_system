...
public class KeybindingController {
    ...
    public ResponseEntity<Void> create(@RequestBody Keybinding keybinding) {
        ...
        final String up = keybinding.getUp();
        final String down = keybinding.getDown();
        final String left = keybinding.getLeft();
        final String right = keybinding.getRight();

        final long id = keybindingService.create(up, down, left, right);
        final String location = String.format("/keybinding/%d", id);
